  ...
  //Populates dropdown
    function populateDropdown(query, results) {
      ...
      if(!settings.allowRepeat) {
        ...
        _.each(mentionsCollection, function(item){
          if(item.type == 'topic'){
            var resName = "";
            var itemNames = item.name.replace("#[","").replace("]","").split(" ");
            _.each(itemNames, function(itemName){
              resName += " "+itemName.replace(itemName[0], itemName[0].toUpperCase());
            });
            item.value = resName.trim();
          }
        });
  ...
  //Resets the text area
    function resetInput() {
      if(settings.editMessage.length){
        elmInputBox.val($("<div/>").html(settings.editMessage).text());
      }else{
        elmInputBox.val('');
      }
